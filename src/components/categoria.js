...
router.get('/Categoria', function (req, res) {
  ...
  oConnection.connect(err => {
    oConnection.db('Recycle').collection('categoria').find({ isActive: 1 }).toArray((err, result) => {
      if (err) throw err;
...
router.get('/CategoriaById', function (req, res) {
  ...
  oConnection.connect(err => {
    oConnection.db('Recycle').collection('categoria').find(oFiltro).toArray((err, result) => {
      if (err) throw err;
...
router.post('/Categoria', function (req, res) {
  ...
  oConnection.connect(err => {
    oConnection.db('Recycle').collection('categoria').insertOne(oDatos, function (error, response) {
      if (error) {
...
router.put('/Categoria', function (req, res) {
  ...
  oConnection.connect(err => {
    oConnection.db('Recycle').collection('categoria').replaceOne(oFiltro, oDatos, function (error, response) {
      if (error) {
        console.log('Error occurred while inserting');
        // return 
      } else {
...
router.put('/CategoriaDelete', function (req, res) {
  ...
  oConnection.connect(err => {
    oConnection.db('Recycle').collection('categoria').updateOne(oFiltro, oDatos, function (error, response) {
      if (error) {
        console.log('Error occurred while inserting');
        // return 
      } else {
